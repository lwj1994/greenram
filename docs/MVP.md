...
目标：当 macOS 内存压力升高时，自动退出不在前台、不在白名单里的可清理 App。
...
- 清理判断：可清理 / 不可清理。
- 自动结束可清理 App：阈值命中后直接强制结束清理候选 App。
...
第一版不是“清理内存神器”，而是一个硬阈值可清理 App 强制结束器。核心能力只有一个：超过 RAM / Swap 上限时，强制结束可清理 App。
